...
def predict_face(img_path_or_matrix) -> dict:
    dict_output = {
        'confidence': '',
        'result': '',
        'bboxes': ''
...
def predict_face_b64(img_b64str: str, b64_return: bool) -> dict:
    dict_output = {
        'bboxes': '',
        'confidence': '',
        'result': '',
    ...
    results = model.predict(orgimg, max_det=50, conf=0.4, verbose=False)  
    ...
    if dict_output['result']:
        if b64_return:
            ...
            for each_box, each_conf in zip(dict_output['bboxes'], dict_output['confidence']):
                ...
                cv2.putText(img,f"Face: {round(each_conf* 100,2)}%",(int(x1), int(y1) - 10),fontFace = cv2.FONT_HERSHEY_SIMPLEX,fontScale = 0.6,color = annotation_color,thickness=2)
